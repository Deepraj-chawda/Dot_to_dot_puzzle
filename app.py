...
class GUI:
    def __init__(self, master):
        ...
        self.image_frames = [
            ...
            LabelFrame(
                master, width=self.size[0]//3, height=self.image_frames_height).grid(row=1, column=2)
        ]
        ...
        Button(master, text="Generate Image", width=43,
    ...
    def open(self):
        ...
        self.img = ImageTk.PhotoImage(Image.open(self.file_path).resize((self.size[0]//3,
                                                                         self.image_frames_height), Image.ANTIALIAS))
        self.images[0].config(image=self.img)
        self.images[0].grid(row=1, column=0)

    def black_and_white(self):
        bk.Image_converter().load_img(self.file_path)
        self.blc_img = ImageTk.PhotoImage(Image.open("ResultImages/black_white.png").resize((self.size[0]//3,
                                                                                             self.image_frames_height), Image.ANTIALIAS))
        self.images[1].config(image=self.blc_img)
        self.images[1].grid(row=1, column=1)

    def dotted_image(self):
        bk.Image_converter().generate(self.file_path)
        self.dot_img = ImageTk.PhotoImage(Image.open("ResultImages/edge.png").resize((self.size[0]//3,
                                                                                      self.image_frames_height), Image.ANTIALIAS))
        self.images[2].config(image=self.dot_img)
        self.images[2].grid(row=1, column=2)
